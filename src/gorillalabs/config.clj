...
(defn form-seq
  ...
  [reader]
  (let [form (edn/read {:readers {'env env
                                  'envf (fn [[fmt & args]]
                                          (apply format fmt
                                            (map env args)))}
                    :eof reader} reader)]
...
(defmulti invoke-extension
          ...
          (fn [_ _ operation & _] operation))

(defn mapify
  "Turns a form read from a config file at `origin` into a map."
  [form origin config]
  (cond
    (map? form) (merge config form)
    (list? form) (let [[operation & args] form]
                   (apply invoke-extension config origin operation args))
...
(defn merge-config
  ...
  [path config-forms]
  (reduce (fn [acc form] (mapify form path acc)) {} config-forms))
...
(defmethod invoke-extension 'include
  [config origin _ path]
  (merge config (read-config (path-relative-to origin path))))

(defmethod invoke-extension 'include-as
  [config origin _ key path]
  (assoc config key (read-config (path-relative-to origin path))))

(defmethod invoke-extension 'assoc-in
  [config _ _ ks value]
  (assoc-in config ks value))
